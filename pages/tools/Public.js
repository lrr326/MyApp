...
//数据请求
export function feach_request(api,header,data,navigation){
    return new Promise((resolve,reject)=>{
        fetch(URL+api,{
            ...
            if(response.headers.map['x-csrf-token']){
                const param = {
                    'token':response.headers.map['x-csrf-token'],
                    'cookie':response.headers.map['set-cookie']
                }
                AsyncStorage.setItem('option',JSON.stringify(param));
            }
            return response.json()
        }).then(responseData=>{
                if(responseData.data.code==10002){
                    ToastAndroid.showWithGravity(
                        '登录过期，请重新登录～',
                        ToastAndroid.SHORT,
                        ToastAndroid.CENTER
                    );
                    AsyncStorage.removeItem('option', function (error) {
                        if (error) {
                        }else {
